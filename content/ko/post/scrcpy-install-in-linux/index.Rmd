...
featured: false
image:
  caption: 'Image credit: [scrcpy in action](https://raw.githubusercontent.com/issactoast/theissacleedotcom/master/static/img/scrcpy.png)'
  focal_point: ""
  placement: 2
  preview_only: true
lastmod: '`r format(Sys.Date(), "%Y-%B-%d")`'
projects: [linux]
...
1. 첫번째 파일이 미리 빌트된 서버 파일이다. 이 파일의 확장자를 `.jar`로 바꿔주고 다음의 명령어를 터미널에서 실행.

```{r, engine = 'bash', eval = FALSE}
sudo install scrcpy-server-v*.jar /usr/local/bin/scrcpy-server.jar
```

1. 두번째 파일의 압축을 풀어주고, 풀어진 결과 폴더의 이름을 `scrcpy`로 바꿔준다. 다음의 명령어를 차례로 실행한다. 내용은 압축 풀어진 `scrcpy` 폴더로 이동 후 `meson` 명령어를 이용해 `build` 폴더를 생성함. 그 후 `build` 폴더로 이동 후 설치. 

```{r, engine = 'bash', eval = FALSE}
cd scrcpy

meson x --buildtype release --strip -Db_lto=true -Dprebuilt_server=../scrcpy-server-v#.##.#.jar
