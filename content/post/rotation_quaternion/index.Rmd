...
\end{align*}
$$
Note that the first line in the fomula shows that the rotating the frame has the same effect of rotating the objects opposite direction. The defined matrix $C_{\theta}^{B|I}$ called direction cosine matrix (DCM). Why do we call this matrix DCM eventhough we have cos and sin function in the matrix? The answer can be shown with the conversion of sine function to the cosine function using the identities as follows:
$$
\begin{align*}
...
Thus, in our example, we have the following values for $\theta$s:
$$
\left(\begin{array}{cc}
cos\left(\theta_{11}\right) & cos\left(\theta_{12}\right)\\
cos\left(\theta_{21}\right) & cos\left(\theta_{22}\right)
...
We can check out the values in the matrix are the same as before.
...
To summarize the discussion so far, coordinates of a given vector $\underline{a}$ in the initial frame can be transfer to the rotated frame by using the following matrix multiplication.
$$
